...
public class PluginInterfaceTestResultsHandler
{
    public static QueryResult<BaseItemDto> BuildResults(HomeScreenSectionPayload payload)
    {
        return new QueryResult<BaseItemDto>([
            ...
            {
                Id = Guid.NewGuid(),
                Name = "From Reflection Handler",
                Overview = payload.AdditionalData,
            },
        ]);
    }
}
...
public class PluginInterfaceTests
{
    ...
    public void RegisterSection_invokes_reflection_handler_from_loaded_assembly()
    {
        ...
        _fixture
            .HomeScreenManagerMock.Setup(manager => manager.RegisterResultsDelegate(It.IsAny<PluginDefinedSection>()))
    ...
    public void RegisterSection_posts_to_results_endpoint_when_no_assembly_given()
    {
        ...
        using JellyseerrFakeServer server = JellyseerrFakeServer.Start(path =>
            (200, JsonConvert.SerializeObject(serverResult))
        );

        int port = new Uri(server.BaseUrl).Port;
        _fixture.ServerApplicationHostMock.SetupGet(host => host.HttpPort).Returns(port);
