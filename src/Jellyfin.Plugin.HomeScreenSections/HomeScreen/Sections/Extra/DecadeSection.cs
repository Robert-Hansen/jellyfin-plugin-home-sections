...
public class DecadeSection : IHomeScreenSection
{
    ...
    {
        QueryResult<BaseItem> sample = _libraryManager.GetItemsResult(
            new InternalItemsQuery(user)
            {
                IncludeItemTypes = [BaseItemKind.Movie],
                Recursive = true,
        ...
        return sample
            .Items.Where(x => x.ProductionYear is >= 1900 and <= 2100)
    ...
    private static bool? GetHideWatchedIsPlayed()
    {
        SectionSettings? settings = HomeScreenSectionsPlugin.Instance?.Configuration?.SectionSettings.FirstOrDefault(
            x => string.Equals(x.SectionId, "Decade", StringComparison.Ordinal)
        );
